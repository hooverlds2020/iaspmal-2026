...
import { Download, Eye, Check, X, RefreshCw, Search, QrCode, ExternalLink, Award, Lock, WifiOff, AlertTriangle, Send, FileSpreadsheet } from 'lucide-react';
...
const RegistrationsDashboard = () => {
  ...
  const exportToCSV = () => {
    // Usamos 'filteredRegistrations' para exportar lo que el usuario ve en pantalla
    ...
    const csvRows = [
        ...
        ...dataToExport.map(row => {
            // Limpiar datos para evitar romper el CSV (quitar comas y saltos de línea)
            ...
            return [
                ...
                clean(row.email),
                clean(row.country),
                clean(getCategoryLabel(row.category)),
                clean(row.status),
                row.payment_amount || 0,
                row.payment_currency || 'MXN',
                clean(row.payment_method),
                clean(row.payment_date),
                clean(row.attendance_code),
                row.attendance_confirmed ? "SI" : "NO",
                clean(row.attendance_date),
                clean(row.notes)
            ].join(",");
        })
    ].join("\n");

    // 3. Crear el archivo Blob con BOM para que Excel lea acentos (UTF-8)
    const bom = "\uFEFF"; 
    ...
    link.href = url;
    link.setAttribute("download", `Registros_IASPM26_${filter}_${new Date().toISOString().slice(0,10)}.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    
    toast.success(`Exportados ${dataToExport.length} registros a Excel`);
  };

  // --- GENERAR PDF (CONSTANCIA) ---
  ...
  const updateStatus = async (id, newStatus) => {
    if (newStatus === 'paid' && (!paymentAmount || parseFloat(paymentAmount) <= 0)) {
      toast.warning('Por favor ingresa el monto del pago para continuar.');
      return;
    }
    ...
    const promise = async () => {
      ...
      if (newStatus === 'paid') {
        await sendPaymentApproval(registration.email, registration.full_name, registration.attendance_code, registration.category);
      } else if (newStatus === 'rejected') {
        await sendRejectionNotice(registration.email, registration.full_name);
      }
      
      setPaymentAmount(''); 
      return newStatus === 'paid' ? 'Pago aprobado y correo enviado' : 'Registro rechazado correctamente';
    };
  ...
  const confirmAttendance = async (id) => {
    ...
    if (registration.status !== 'paid') {
      toast.error('No se puede confirmar: El participante no tiene el pago aprobado.');
      return;
    }
    ...
    toast.promise(promise, {
      ...
      success: '¡Asistencia confirmada con éxito!',
  ...
  const getCategoryLabel = (cat) => {
    const map = { 'sur_global': 'Sur Global', 'norte_global': 'Norte Global', 'institucion_convocante': 'Institución Convocante', 'estudiante': 'Estudiante', 'asistente': 'Asistente' };
    return map[cat] || cat;
  };

  const getStatusBadge = (status) => {
    const styles = { pending: 'bg-yellow-100 text-yellow-800 border-yellow-200', paid: 'bg-green-100 text-green-800 border-green-200', rejected: 'bg-red-100 text-red-800 border-red-200' };
  ...
  const filteredRegistrations = registrations.filter(reg => 
    ...
    (reg.attendance_code && reg.attendance_code.toLowerCase().includes(searchTerm.toLowerCase()))
  );
              ...
              {['all', 'pending', 'paid'].map(f => (
                <button
                  ...
                  className={`px-4 py-2 rounded-lg text-sm font-medium transition whitespace-nowrap ${
                    filter === f ? 'bg-teal-600 text-white shadow-md' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                  }`}
                >
                  {f === 'all' ? 'Todos' : f === 'pending' ? 'Pendientes de Pago' : 'Pagados'}
                </button>
              ))}
            </div>
            ...
            <div className="flex gap-3 w-full md:w-auto">
              <div className="relative flex-1 md:w-64">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-teal-500 focus:border-transparent outline-none transition"
                />
              </div>
              ...
              <button
                onClick={exportToCSV}
                className="bg-green-700 hover:bg-green-800 text-white px-4 py-2 rounded-lg transition flex items-center gap-2 shadow-sm"
              ...
              <button
                ...
                className="p-2 text-gray-600 hover:bg-gray-100 rounded-lg transition border border-gray-300"
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden min-h-[400px]">
          {loading ? (
             ...
             <div className="flex flex-col items-center justify-center py-20 text-center">
                ...
                <h3 className="text-xl font-bold text-gray-700">Error de conexión</h3>
                <p className="text-gray-500 mb-6 max-w-md">No pudimos cargar los registros. Verifica tu conexión a internet o intenta nuevamente.</p>
                <button onClick={() => fetchRegistrations()} className="bg-teal-600 text-white px-6 py-2 rounded-lg hover:bg-teal-700 transition">
                    Reintentar
                </button>
             </div>
             ...
             <div className="flex flex-col items-center justify-center py-20 text-center">
                ...
                <p className="text-gray-500 text-sm">Intenta ajustar tu búsqueda o los filtros.</p>
             </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-gray-50 border-b border-gray-200">
                  <tr>
                    <th className="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Participante</th>
                    ...
                    <th className="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Asistencia</th>
                  ...
                  {filteredRegistrations.map((reg) => (
                    <tr key={reg.id} className="hover:bg-gray-50 transition group">
                      <td className="px-6 py-4">
                        ...
                        <div className="text-xs text-gray-400 mt-0.5">{reg.country}</div>
                      </td>
                      ...
                      <td className="px-6 py-4 text-center">
                        <button 
                            ...
                            className="text-teal-600 hover:text-teal-800 hover:bg-teal-50 px-3 py-1.5 rounded-md text-sm font-medium transition flex items-center justify-center gap-1 mx-auto"
                        >
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
            <div className="bg-white rounded-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl">
              ...
              <div className="sticky top-0 bg-white border-b px-6 py-4 flex justify-between items-center z-10">
                ...
                <button onClick={() => setSelectedRegistration(null)} className="p-2 hover:bg-gray-100 rounded-full transition">
              ...
              <div className="p-6 space-y-8">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <div className="col-span-2">
                                ...
                                {selectedRegistration.payment_proof_url ? (
                                    <a href={selectedRegistration.payment_proof_url} target="_blank" rel="noopener noreferrer" className="text-sm text-teal-600 hover:underline flex items-center gap-1">
                                        <Download className="w-3 h-3" /> Ver archivo
                                    </a>
                                ) : <span className="text-sm text-gray-400">No adjuntado</span>}
                            </div>
                        </div>
                    </div>
                        ...
                        {selectedRegistration.status !== 'paid' && (
                            <div className="space-y-3">
                                <div className="grid grid-cols-2 gap-2">
                                    <input type="number" placeholder="Monto" value={paymentAmount} onChange={e => setPaymentAmount(e.target.value)} className="text-sm border rounded px-2 py-1" />
                                    <select value={paymentCurrency} onChange={e => setPaymentCurrency(e.target.value)} className="text-sm border rounded px-2 py-1"><option value="MXN">MXN</option><option value="USD">USD</option></select>
                                </div>
                                <div className="flex gap-2">
                                    <button onClick={() => updateStatus(selectedRegistration.id, 'paid')} className="flex-1 bg-green-600 text-white py-1.5 rounded text-sm hover:bg-green-700 transition">Aprobar</button>
                                    <button onClick={() => updateStatus(selectedRegistration.id, 'rejected')} className="flex-1 bg-white border border-red-200 text-red-600 py-1.5 rounded text-sm hover:bg-red-50 transition">Rechazar</button>
                                </div>
                            </div>
                        )}
                        {selectedRegistration.status === 'paid' && (
                            <div className="space-y-2">
                                <div className="text-xs text-green-700 bg-green-50 p-2 rounded border border-green-100">
                                    Pago aprobado el {selectedRegistration.payment_date || 'fecha desconocida'}.
                                </div>
                ...
                <div className={`border-2 rounded-xl p-5 ${selectedRegistration.attendance_confirmed ? 'border-blue-200 bg-blue-50/50' : 'border-gray-200'}`}>
                    <div className="flex flex-col md:flex-row gap-6 items-center">
                        ...
                        <div className="flex-1 text-center md:text-left space-y-3">
                            <div>
                                <h3 className="text-lg font-bold text-gray-800">Control de Acceso</h3>
                                <p className="text-sm text-gray-500">Escanea el QR para registrar entrada</p>
                            </div>
                            ...
                            {selectedRegistration.attendance_confirmed ? (
                                <div className="inline-flex items-center gap-2 px-4 py-2 bg-green-100 text-green-800 rounded-lg font-bold">
                                    <Check className="w-5 h-5" /> Entrada Registrada
                                </div>
                            ) : (
                                selectedRegistration.status === 'paid' ? (
                                    <button onClick={() => confirmAttendance(selectedRegistration.id)} className="w-full md:w-auto bg-blue-600 text-white px-6 py-2 rounded-lg font-semibold hover:bg-blue-700 transition shadow-sm">
                                    ...
                                    <div className="flex items-center gap-2 text-amber-600 bg-amber-50 px-3 py-2 rounded-lg text-sm border border-amber-200">
                                        ...
                                        <span>Requiere pago para registrar asistencia</span>
                                    </div>
                                )
                            )}
                        </div>
                ...
                <div className="flex items-center justify-between p-4 bg-gray-50 rounded-xl border border-gray-200">
                    <div className="flex items-center gap-3">
                        <div className={`p-2 rounded-lg ${selectedRegistration.attendance_confirmed && selectedRegistration.status === 'paid' ? 'bg-amber-100 text-amber-600' : 'bg-gray-200 text-gray-400'}`}>
                            <Award className="w-6 h-6" />
                        </div>
                        <div>
                            <h4 className="font-bold text-gray-800">Certificado Digital</h4>
                            <p className="text-xs text-gray-500">Disponible tras completar pago y asistencia.</p>
                        </div>
                    </div>
                    {selectedRegistration.attendance_confirmed && selectedRegistration.status === 'paid' ? (
                        <button onClick={() => generateCertificate(selectedRegistration)} className="text-teal-600 hover:text-teal-800 font-semibold text-sm flex items-center gap-1 border border-teal-200 px-3 py-1.5 rounded-lg bg-white hover:bg-teal-50 transition">
                            <Download className="w-4 h-4" /> Descargar
                        </button>
                    ) : (
                        <span className="text-xs font-semibold text-gray-400 bg-gray-100 px-3 py-1 rounded-lg border border-gray-200 flex items-center gap-1">
                ...
                {/* NOTAS */}
                <div>
                    <label className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1 block">Notas Privadas</label>
                    <textarea 
                        ...
                        rows={2}
                        placeholder="Escribe notas internas sobre este participante..."
                    />
                </div>
