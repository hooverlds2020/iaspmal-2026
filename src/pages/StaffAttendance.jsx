...
import { Lock, UserCheck, Search, CheckCircle, XCircle, AlertTriangle, RefreshCw, X, LogOut, QrCode } from 'lucide-react';
...
const StaffAttendance = () => {
  ...
  // MEJORA: Detección más robusta del código URL
  useEffect(() => {
    if (isAuthenticated && urlCode && !processing && !lastScan) {
        // Pequeño delay para asegurar que la UI está lista
        const timer = setTimeout(() => {
            toast.info(`Leyendo QR: ${urlCode}`); // Feedback visual
            processCode(urlCode);
        }, 500);
        return () => clearTimeout(timer);
    }
  }, [isAuthenticated, urlCode]); // Dependemos del string urlCode, no del objeto params
  ...
  const resetScanner = () => {
    setLastScan(null);
    setCode('');
    setSearchParams({}); // Limpia la URL para evitar bucles
  ...
  const processCode = async (inputCode) => {
    ...
    cleanCode = cleanCode.replace(/^IASP[-]?/, ''); // Quitamos prefijo si existe

    console.log("Procesando código:", cleanCode);

    try {
      const { data, error } = await supabase.rpc('mark_attendance_secure', { qr_code: cleanCode });
      ...
      if (data.success) {
        ...
        resultData = { type: 'error', msg: 'DENEGADO', sub: `Código: ${cleanCode}`, name: 'No encontrado' };
      }
      ...
      setProcessing(false);
      setCode('');
      // Si fue por URL, damos un poco más de tiempo para leer
      const delay = urlCode ? 5000 : 3500;
      timerRef.current = setTimeout(() => { resetScanner(); }, delay);
    }
  };
    ...
    return (
      <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4">
        <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-sm text-center animate-in fade-in zoom-in duration-300">
          ...
          {/* Debug visual si hay código en URL pero pide PIN */}
          {urlCode && (
              <div className="mt-4 p-2 bg-blue-50 text-blue-600 text-xs rounded border border-blue-100 flex items-center justify-center gap-2">
                  <QrCode className="w-4 h-4"/>
                  <span>QR detectado: <strong>{urlCode}</strong></span>
              </div>
          )}
      ...
      <div className="flex-1 w-full max-w-md mx-auto p-4 flex flex-col justify-center">
        <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-200 relative overflow-hidden">
            ...
            <div className="relative z-10">
                <p className="text-center text-xs text-gray-400 font-bold uppercase tracking-widest mb-6">Ingreso Manual</p>
